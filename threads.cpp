...
std::mutex L1;								// lock on split-point objects. This lock guards SP vars mlp, c_1, slave_bits, and globals thread_running_mask, sp_all_mask. Also SP selection by slaves.
std::condition_variable CV1;					// slaves are allowed to run
...
unsigned int SlaveThreadProc(uint32_t in_slaveId){// slave function - new approach.
	UINT64 bb;
	unsigned int SlaveId=in_slaveId,ply_l,node_type_l,go_to_sleep=0;
	...
	std::unique_lock lockedL1(L1); // lock the split-point
	while(1){ // infinite loop. Here L1 is locked
		...
		lockedL1.unlock();			// release the lock*************************************************************
...
void init_threads(unsigned int TC){// thread count - slaves only.
	unsigned int i,EXTRA_THREADS=std::max(5U,TC/2);
	...
	if( slave_count ){
		slave_terminate=1;
		CV1.notify_all(); // wake the slaves
		for (i = 0; i < slave_count.load(); ++i) {
			s_th[i]->join(); // wait for slaves to terminate
			delete s_th[i];  // need this one
			s_th[i] = nullptr;
		}
	...
	// start threads
	TC=std::min(64U,TC+EXTRA_THREADS); // start several more threads than allowed - to cover sleeping ones.
	// allocate memory
	if( s_th!=NULL ) free(s_th);
	s_th=(std::thread **)malloc((TC+2)*(sizeof(std::thread *)+sizeof(board))); // 4,516(outdated) if MAX_MOVE_HIST=8. For 20 threads this is 25*4,516=111 Kb.
	if( s_th==NULL ) exit(123);
	b_s=(board*)&s_th[TC+2];

	//for(i=1;i<=TC;++i) s_th[i-1]=CreateThread(NULL,0,SlaveThreadProc,(PVOID)i,0,&id);
	for(i=1;i<=TC;++i) s_th[i-1]=new std::thread(SlaveThreadProc, i);
	sleepMs(50);
return;
}
